...
module.exports = (pool) => {
    ...
    router.get("/getEducationInfo/country_name", async (req, res) => {
        try {
            ...
            const result = await pool.query(
                "SELECT i.indicator_name, AVG(e.value) AS indicator_value FROM Country c JOIN Education e ON e.country_code = c.country_code JOIN EducationIndicators i ON e.indicator_code = i.indicator_code WHERE c.country_name LIKE country_name GROUP BY i.indicator_name",
                [country_name]
            )
    ...
    router.get("/getIMFInfo/country_name", async (req, res) => {
        try {
            ...
            const result = await pool.query(
                "SELECT i.indicator_name, AVG(IMF.value) FROM Country c JOIN IMF ON IMF.country_code = c.country_code JOIN IMFIndicators i ON i.indicator_code = IMF.indicator_code WHERE c.country_name LIKE country_name GROUP BY i.indicator_name",
                [country_name]
            )
    ...
    router.get("/getSummaryStockInfo/country_name", async (req, res) => {
        try {
            ...
            const result = await pool.query(
                "WITH AvgCloseLastYear AS (SELECT sd.industry_tag AS industry, c.country_name AS country, AVG(sp.close) AS avg_close_last_year FROM stock_price sp JOIN stock_desc sd ON sp.ticker = sd.ticker JOIN country c ON sd.country_code = c.country_code WHERE sp.date BETWEEN '2022-01-01' AND '2022-12-31' AND c.country_name = country_name GROUP BY sd.industry_tag, c.country_name) SELECT sd.industry_tag AS industry, c.country_name AS country, COUNT(sp.ticker) AS company_count, AVG(sp.open) AS avg_open_price, AVG(sp.close) AS avg_close_price, MAX(sp.close) AS max_close_price, MIN(sp.close) AS min_close_price, ROUND(CAST((AVG(sp.close) - COALESCE(acl.avg_close_last_year, 0)) / NULLIF(acl.avg_close_last_year, 0) * 100 AS NUMERIC), 2) AS yoy_close_growth, SUM(CASE WHEN (sp.high - sp.low) > 10 THEN 1 ELSE 0 END) AS high_volatility_count FROM stock_price sp JOIN stock_desc sd ON sp.ticker = sd.ticker JOIN country c ON sd.country_code = c.country_code LEFT JOIN AvgCloseLastYear acl ON sd.industry_tag = acl.industry AND c.country_name = acl.country WHERE sp.date BETWEEN '2023-01-01' AND '2023-12-31' AND c.country_name = country_name GROUP BY sd.industry_tag, c.country_name, acl.avg_close_last_year HAVING COUNT(sp.ticker) >= 5 ORDER BY industry",
                [country_name]
            )
    ...
    router.get("/getSimilarEducationCountries/country_name", async (req, res) => {
        try {
            ...
            const result = await pool.query(
                "CREATE MATERIALIZED VIEW indicator_stats AS SELECT indicator_code, AVG(value) AS mean_value, STDDEV(value) AS stddev_value FROM education GROUP BY indicator_code; CREATE MATERIALIZED VIEW standardized_education AS (SELECT c.country_name, (e.value - s.mean_value) / NULLIF(s.stddev_value, 0) AS standardized_value FROM education e JOIN indicator_stats s ON e.indicator_code = s.indicator_code JOIN country c ON c.country_code=e.country_code); CREATE MATERIALIZED VIEW country_education_avg AS (SELECT country_name, AVG(standardized_value) AS country_avg_standardized_value FROM standardized_education GROUP BY country_name); SELECT cea.country_name, cea.country_avg_standardized_value AS performance_score, ABS(cea.country_avg_standardized_value - (SELECT country_avg_standardized_value FROM country_education_avg WHERE country_name = country_name)) AS distance FROM country_education_avg cea WHERE cea.country_name <> country_name ORDER BY distance LIMIT 2",
                [country_name]
            )
