...
module.exports = (pool) => {
    ...
    router.get("/getIMFInfo/:country_name", async (req, res) => {
        try {
            ...
            const result = await pool.query(
                "SELECT i.indicator_name, AVG(IMF.value) FROM Country c JOIN IMF ON IMF.country_code = c.country_code JOIN IMFIndicators i ON i.indicator_code = IMF.indicator_code WHERE c.country_name LIKE $1 GROUP BY i.indicator_name",
                [country_name]
            )
